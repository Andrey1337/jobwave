...
@section('content')
<form action="{{ route('resume.update', $resume->id) }}" class="form_register_vacancy" enctype="multipart/form-data" method="post">
    @csrf
    @method('PUT')
    ...
    <div class="mb-3">
        ...
        <input type="text" class="form-control" id="job_title" name="job_title" value="{{ $resume->job_title }}">
    </div>
    ...
    <div class="mb-3">
        ...
        <select class="form-select" id="willing_to_relocate" name="willing_to_relocate">
            <option value="готов к переезду" {{ $resume->willing_to_relocate == 'готов к переезду' ? 'selected' : '' }}>Готов к переезду</option>
            <option value="не готов к переезду" {{ $resume->willing_to_relocate == 'не готов к переезду' ? 'selected' : '' }}>Не готов к переезду</option>
        </select>
    </div>
    ...
    <div class="mb-3">
        ...
        <select class="form-select" id="willing_to_travel" name="willing_to_travel">
            <option value="готов к командировкам" {{ $resume->willing_to_travel == 'готов к командировкам' ? 'selected' : '' }}>Готов к командировкам</option>
            <option value="не готов к командировкам" {{ $resume->willing_to_travel == 'не готов к командировкам' ? 'selected' : '' }}>Не готов к командировкам</option>
        </select>
    </div>
    ...
    <div class="mb-3">
        ...
        @foreach ($employmentTypes as $type)
            <input class="custom_checkbox form-check-input" type="checkbox" name="employment_type[]" value="{{ $type }}" id="{{ $type }}" {{ in_array($type, explode(',', $resume->employment_type)) ? 'checked' : '' }}>
    ...
    <div class="mb-3">
        ...
        @foreach ($workSchedules as $schedule)
            <input class="custom_checkbox form-check-input" type="checkbox" name="work_schedule[]" value="{{ $schedule }}" id="{{ $schedule }}" {{ in_array($schedule, explode(',', $resume->work_schedule)) ? 'checked' : '' }}>
    ...
    <div class="mb-3">
        ...
        <select class="form-select" id="specialization_id" name="specialization_id">
            ...
            @foreach($specializations as $specialization)
                <option value="{{ $specialization->id }}" {{ $specialization->id == $resume->specialization_id ? 'selected' : '' }}>{{ $specialization->name }}</option>
            @endforeach
        </select>
    </div>
    ...
    <div class="mb-3">
        ...
        <select name="profession_id" class="form-select form-control-for-company" id="profession_id">
            ...
            @foreach($professions as $profession)
                <option value="{{ $profession->id }}" {{ $profession->id == $resume->profession_id ? 'selected' : '' }}>{{ $profession->name }}</option>
            @endforeach
        </select>
    </div>
    ...
    <div class="mb-3">
        ...
        <input type="text" class="form-control" id="desired_salary_min" name="desired_salary_min" value="{{ $resume->desired_salary_min }}">
    </div>

    <div class="mb-3">
        <label for="desired_salary_max" class="form-label">Желаемая максимальная зарплата</label>
        <input type="text" class="form-control" id="desired_salary_max" name="desired_salary_max" value="{{ $resume->desired_salary_max }}">
    </div>
    ...
    <div class="education-container">
        ...
        <div class="education-inputs">
            <div class="education-item">
                <label for="start_date_{{ $key }}" class="form-label f12777">Дата начала обучения</label>
                <input type="date" name="educations[{{ $key }}][start_date]" class="form-control mb-1" placeholder="Дата начала" id="start_date_{{ $key }}" value="{{ $education->start_date }}">
                <label for="end_date_{{ $key }}" class="form-label f12777">Дата окончания обучения</label>
                <input type="date" name="educations[{{ $key }}][end_date]" class="form-control mb-1" placeholder="Дата окончания" id="end_date_{{ $key }}" value="{{ $education->end_date }}">
                <input type="text" name="educations[{{ $key }}][institution]" class="form-control mb-3" placeholder="Название учебного заведения" value="{{ $education->institution }}">
...
<div class="mb-3">
    ...
    <div class="work-experience-container">
        @foreach ($resume->workExperiences as $key => $experience)
            @if ($experience)
                <div class="work-experience-inputs">
                    <div class="work-experience-item">
                        <label for="start_date_{{ $key }}" class="form-label f12777">Дата начала работы</label>
                        <input type="date" name="work_experiences[{{ $key }}][start_date]" class="form-control mb-1" placeholder="Дата начала" id="start_date_{{ $key }}" value="{{ $experience ? $experience->start_date : '' }}">
                        <label for="end_date_{{ $key }}" class="form-label f12777">Дата окончания работы</label>
                        <input type="date" name="work_experiences[{{ $key }}][end_date]" class="form-control mb-1" placeholder="Дата окончания" id="end_date_{{ $key }}" value="{{ $experience ? $experience->end_date : '' }}">
                        <input type="text" name="work_experiences[{{ $key }}][company]" class="form-control mb-1" placeholder="Название компании" value="{{ $experience ? $experience->company_name : '' }}">
                        <input type="text" name="work_experiences[{{ $key }}][position]" class="form-control mb-1" placeholder="Должность" value="{{ $experience ? $experience->position : '' }}">
                        <textarea name="work_experiences[{{ $key }}][description]" class="form-control mb-3" placeholder="Описание">{{ $experience ? $experience->description : '' }}</textarea>
    ...
    <div class="mb-3">
        ...
        <textarea class="form-control" id="about_me" name="about_me">{{ $resume->about_me }}</textarea>
    </div>

    <div class="mb-3">
        <label for="citizenship" class="form-label">Гражданство</label>
        <input type="text" class="form-control" id="citizenship" name="citizenship" value="{{ $resume->citizenship }}">
    </div>

    <div class="mb-3">
        <label for="commute_time" class="form-label">Время до работы</label>
        <select class="form-select" id="commute_time" name="commute_time">
            <option value="менее часа" {{ $resume->commute_time == 'менее часа' ? 'selected' : '' }}>Менее часа</option>
            <option value="около часа" {{ $resume->commute_time == 'около часа' ? 'selected' : '' }}>Около часа</option>
            <option value="не имеет значения" {{ $resume->commute_time == 'не имеет значения' ? 'selected' : '' }}>Не имеет значения</option>
        </select>
    </div>
    ...
    <div class="mb-3">
        ...
        <ul id="selected-skills-list" class="list-group mt-1">
            @foreach($selectedSkills as $skill)
                <li class="selected-skill m-1 list-group-item d-flex justify-content-between align-items-center" data-skill-id="{{ $skill->id }}" style="background-color: #C3EDFF; border-radius: 4px; padding: 5px 12px;">
                    {{ $skill->name }}
                    <button type="button" class="btn btn-sm" onclick="removeSelectedSkill({{ $skill->id }})"><i class="bi bi-x"></i></button>
                </li>
            @endforeach
        </ul> <!-- Список выбранных навыков -->
        <input type="hidden" name="selected_skills_array" id="selected_skills_array" value="{{ json_encode($selectedSkills->pluck('id')) }}">
    </div>
    ...
    <div class="mb-3">
        ...
        <ul id="selected-languages-list" class="list-group mt-1">
            @foreach($resume->languages as $language)
                <li class="selected-language m-1 list-group-item d-flex justify-content-between align-items-center" data-language-id="{{ $language->id }}" style="background-color: #C3EDFF; border-radius: 4px; padding: 5px 12px;">
                    {{ $language->name }}
                    <button class="btn btn-sm" onclick="removeSelectedLanguage({{ $language->id }})"><i class="bi bi-x"></i></button>
                </li>
            @endforeach
        </ul> <!-- Список выбранных языков -->
        <input type="hidden" name="selected_languages_array" id="selected_languages_array" value="{{ json_encode($resume->languages->pluck('id')) }}">
    </div>
    ...
    <button type="submit" class="btn_stock_main">Сохранить изменения</button>
    </form>

    <script>
        const selectedSkillsArray = JSON.parse(document.getElementById('selected_skills_array').value);

        // Функция для удаления выбранного навыка из списка
        function removeSelectedSkill(skillId) {
            const selectedSkillsList = document.getElementById('selected-skills-list');
            const selectedSkillItem = selectedSkillsList.querySelector(`[data-skill-id="${skillId}"]`);
            if (selectedSkillItem) {
                selectedSkillItem.remove();
                const updatedSelectedSkillsArray = selectedSkillsArray.filter(id => id !== skillId);
                document.getElementById('selected_skills_array').value = JSON.stringify(updatedSelectedSkillsArray);
            }
        }

        // Получение списка выбранных языков с сервера
        const selectedLanguagesArray = JSON.parse(document.getElementById('selected_languages_array').value);
        ...
        function removeSelectedLanguage(languageId) {
            ...
            const selectedLanguageItem = selectedLanguagesList.querySelector(`[data-language-id="${languageId}"]`);
            if (selectedLanguageItem) {
                selectedLanguageItem.remove();
                const updatedSelectedLanguagesArray = selectedLanguagesArray.filter(id => id !== languageId);
                document.getElementById('selected_languages_array').value = JSON.stringify(updatedSelectedLanguagesArray);
            }
        }
